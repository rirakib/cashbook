    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{route('expense.category')}}" method="post">
                    @csrf
...
<div class="modal expense fade" id="expense" tabindex="-1" role="dialog" aria-labelledby="expense" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="expense">Add Expense</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
                ...
                <form action="{{route('expense.store')}}" method="post">
                    ...
                    <div class="form-group">
                        <select class="custom-select" id="expense_category"
                            aria-label="Example select with button addon" name="expense_category">
                            <option>Choose Category</option>
                            @foreach(DB::table('expense_categories')->get() as $cat)
                            <option value="{{$cat->id}}">{{$cat->expense_category}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
